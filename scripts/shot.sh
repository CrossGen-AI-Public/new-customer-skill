...
# WebGL is forced on through SwiftShader so a three.js hero renders instead of a blank canvas.
set -euo pipefail
...
url="$SRC"; case "$SRC" in http*) ;; *) path="${SRC%%#*}"; frag="${SRC#"$path"}"; url="file://$(cd "$(dirname "$path")" && pwd)/$(basename "$path")$frag";; esac
"$CH" --headless=new --use-angle=swiftshader --enable-unsafe-swiftshader --ignore-gpu-blocklist --allow-file-access-from-files --hide-scrollbars --window-size="${SIZE/x/,}" --virtual-time-budget=9000 --screenshot="$OUT" "$url" 2>/dev/null
echo "wrote $OUT ($SIZE)"
